...
256 & 18.18 (ms) & 14.66 (ms) & 13.76 (ms) & 13.85 (ms) & 18.35 (ms)  \\ \hline
512 & 0.153 (s) & 0.118 (s) & 0.112 (s) & 0.135 (s) & 0.144 (s)  \\ \hline
1024 & 1.248 (s) & 1.022 (s) & 1.069 (s) & 1.249 (s) & 1.414 (s)  \\ \hline
2048 & 19.54 (s) & 10.02 (s) & 9.907 (s) & 11.62 (s) & 14.36 (s)  \\ \hline
 
\end{tabular}\newline


% float: storage size: 4 byte

% BS=4: 4*4*4 = 64 B
% BS=8: 8*8*4 = 256 B
% BS=16: 16*16*4 = 1024 B
% BS=32: 32*32*4 =  4096 B
% BS=64: 64*64*4 = 16384 B
% BS=128: 128*128*4 = 65.536 B 
% BS=256: 256*256*4 = 262.144 B

% blocking bis auf kleinste stufe (2,4) macht wenig sinn, matrizen zwar klein, aber kein 
% performance gewinn mehr... selbst 8*8 oder 16*16 passen in l1 cache und lassen sich effizient
% verarbeiten

% blocking erst ab 128 sinnvoll, siehe Tabelle oben, bei N=64 sogar langsamer als std.algor

% BS <= 256 : eher nicht sinnvoll, kein nennenswerter performance gewinn, matrizen so klein,
% dass diese bereits ohne blocking schon in cpu cache passen


\section{Zusammenfassung}
% vergleich zwischen cache <-> avx, kombination sinnvoll, cache/blocking evtl mehrfach
% BLAS nonplusultra, wiki: erklärung optimierungen  (interne impl.)
% AVX 512, AVX 2... aber prozessorspez.

% einzelne verfahren würden sich ggf. noch weiter optimieren lassen
% von BLAS zu restlichen Verfahren
% BLAS allg. weit entfernt von anderen verfahren, war aber zu erwarten

% parallel: mehrere kerne …
