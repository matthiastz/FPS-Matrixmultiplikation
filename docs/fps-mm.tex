...
Des Weiteren werden die Werte aus
den Ausgangsmatrizen mehrfach verwendet, um \bspw die erste Zeile der Ergebnismatrix $C$ zu berechnen,
benötigen wir für jedes einzelne Element $c$ den gleichen Zeilenvektor aus $A$, das gilt für die
...
zu verbessern. So könnte man die \texttt{for()}-Schleifen so umstrukturieren, dass man diese in kleinere
...
und Parametergrößen als Zweierpotenzen vorgenommen.\newline
...
Der Standardalgorithmus birgt keine großen Überraschungen und wurde in Anlehnung an die 
...
und anderen Optimierungsmöglichkeiten\footnote{Automated Empirical Optimization of Software (AEOS):
Parameterization, Multiple Implementation, Code Generation} Gebrauch, ein großer Vorteil
